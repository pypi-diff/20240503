# Comparing `tmp/fazah-3.12.tar.gz` & `tmp/fazah-3.13.tar.gz`

## filetype from file(1)

```diff
@@ -1 +1 @@
-gzip compressed data, was "fazah-3.12.tar", last modified: Fri May  3 06:04:49 2024, max compression
+gzip compressed data, was "fazah-3.13.tar", last modified: Fri May  3 06:25:22 2024, max compression
```

## Comparing `fazah-3.12.tar` & `fazah-3.13.tar`

### file list

```diff
@@ -1,16 +1,17 @@
-drwxr-xr-x   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:04:49.469795 fazah-3.12/
-drwxr-xr-x   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:04:49.468575 fazah-3.12/Library/
--rw-r--r--   0 aidenlang   (502) staff       (20)        0 2024-04-30 23:12:26.000000 fazah-3.12/Library/__init__.py
--rw-r--r--   0 aidenlang   (502) staff       (20)      390 2024-05-03 06:04:49.469614 fazah-3.12/PKG-INFO
-drwxr-xr-x   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:04:49.469445 fazah-3.12/fazah.egg-info/
--rw-r--r--   0 aidenlang   (502) staff       (20)      390 2024-05-03 06:04:49.000000 fazah-3.12/fazah.egg-info/PKG-INFO
--rw-r--r--   0 aidenlang   (502) staff       (20)      232 2024-05-03 06:04:49.000000 fazah-3.12/fazah.egg-info/SOURCES.txt
--rw-r--r--   0 aidenlang   (502) staff       (20)        1 2024-05-03 06:04:49.000000 fazah-3.12/fazah.egg-info/dependency_links.txt
--rw-r--r--   0 aidenlang   (502) staff       (20)       34 2024-05-03 06:04:49.000000 fazah-3.12/fazah.egg-info/requires.txt
--rw-r--r--   0 aidenlang   (502) staff       (20)       14 2024-05-03 06:04:49.000000 fazah-3.12/fazah.egg-info/top_level.txt
--rw-r--r--   0 aidenlang   (502) staff       (20)       38 2024-05-03 06:04:49.469829 fazah-3.12/setup.cfg
--rw-r--r--   0 aidenlang   (502) staff       (20)      519 2024-05-03 06:04:35.000000 fazah-3.12/setup.py
-drwxr-xr-x   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:04:49.469312 fazah-3.12/tests/
--rw-r--r--   0 aidenlang   (502) staff       (20)        0 2024-04-30 23:12:26.000000 fazah-3.12/tests/__init__.py
--rw-r--r--   0 aidenlang   (502) staff       (20)     1212 2024-05-03 04:03:26.000000 fazah-3.12/tests/test.openai.py
--rw-r--r--   0 aidenlang   (502) staff       (20)     1314 2024-05-03 04:03:26.000000 fazah-3.12/tests/test_gemini.py
+drwxr-xr-x   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:25:22.916292 fazah-3.13/
+drwxr-xr-x   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:25:22.914332 fazah-3.13/Fazah/
+-rw-r--r--   0 aidenlang   (502) staff       (20)     1219 2024-05-03 06:20:07.000000 fazah-3.13/Fazah/Fazah.py
+-rw-r--r--   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:20:07.000000 fazah-3.13/Fazah/__init__.py
+-rw-r--r--   0 aidenlang   (502) staff       (20)      387 2024-05-03 06:25:22.916069 fazah-3.13/PKG-INFO
+drwxr-xr-x   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:25:22.915861 fazah-3.13/fazah.egg-info/
+-rw-r--r--   0 aidenlang   (502) staff       (20)      387 2024-05-03 06:25:22.000000 fazah-3.13/fazah.egg-info/PKG-INFO
+-rw-r--r--   0 aidenlang   (502) staff       (20)      245 2024-05-03 06:25:22.000000 fazah-3.13/fazah.egg-info/SOURCES.txt
+-rw-r--r--   0 aidenlang   (502) staff       (20)        1 2024-05-03 06:25:22.000000 fazah-3.13/fazah.egg-info/dependency_links.txt
+-rw-r--r--   0 aidenlang   (502) staff       (20)       34 2024-05-03 06:25:22.000000 fazah-3.13/fazah.egg-info/requires.txt
+-rw-r--r--   0 aidenlang   (502) staff       (20)       12 2024-05-03 06:25:22.000000 fazah-3.13/fazah.egg-info/top_level.txt
+-rw-r--r--   0 aidenlang   (502) staff       (20)       38 2024-05-03 06:25:22.916336 fazah-3.13/setup.cfg
+-rw-r--r--   0 aidenlang   (502) staff       (20)      516 2024-05-03 06:22:27.000000 fazah-3.13/setup.py
+drwxr-xr-x   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:25:22.915162 fazah-3.13/tests/
+-rw-r--r--   0 aidenlang   (502) staff       (20)        0 2024-05-03 06:20:07.000000 fazah-3.13/tests/__init__.py
+-rw-r--r--   0 aidenlang   (502) staff       (20)     1212 2024-05-03 06:20:07.000000 fazah-3.13/tests/test.openai.py
+-rw-r--r--   0 aidenlang   (502) staff       (20)     1320 2024-05-03 06:22:39.000000 fazah-3.13/tests/test_gemini.py
```

### Comparing `fazah-3.12/setup.py` & `fazah-3.13/setup.py`

 * *Files 2% similar despite different names*

```diff
@@ -1,13 +1,13 @@
 from setuptools import setup, find_packages
 
 setup(
     name='fazah',
-    version='3.12',
-    description='A library that enhances the multilingual capabilities of Large Language Models by manipulating model input, allowing responses in all languages to be equally as coherent.',
+    version='3.13',
+    description='A library that enhances the multilingual capabilities of Large Language Models by manipulating model input, allowing responses in all languages to be equally coherent.',
     author='Aiden Lang, Will Foster',
     author_email='ajlang5@wisc.edu, wjfoster2@wisc.edu',
     packages=find_packages(),
     install_requires=[
         'deep_translator',
         'langdetect',
         'openai',
```

### Comparing `fazah-3.12/tests/test.openai.py` & `fazah-3.13/tests/test.openai.py`

 * *Files identical despite different names*

### Comparing `fazah-3.12/tests/test_gemini.py` & `fazah-3.13/tests/test_gemini.py`

 * *Files 7% similar despite different names*

```diff
@@ -1,8 +1,8 @@
-from Fazah import Fazah
+from Fazah.Fazah import Fazah
 import google.generativeai as genai
 
 # Set up the Google Gemini API key
 API_KEY = "AIzaSyDNTqJoy4ZSLagfj8eD9HiKgPU2h-gUsGw"
 genai.configure(api_key=API_KEY)
 
 # Create an instance of the Google Gemini model
```

